...
sed -i 's/<StackMaxSize>0x100000</StackMaxSize>/<StackMaxSize>0x1000000</StackMaxSize>/g' $WAMR_PATH/product-mini/platforms/linux-sgx/enclave-sample/Enclave/Enclave.config.xml
sed -i 's/<HeapMaxSize>0x8000000<\HeapMaxSize>/<HeapMaxSize>0x20000000<\HeapMaxSize>/g' $WAMR_PATH/product-mini/platforms/linux-sgx/enclave-sample/Enclave/Enclave.config.xml
# switch from simulation mode to hardware mode
# but set prelease mode to true
sed -i 's/SGX_MODE ?= SIM/SGX_MODE ?= HW/g' $WAMR_PATH/product-mini/platforms/linux-sgx/enclave-sample/Makefile
sed -i '/SPEC_TEST ?= 0/a SGX_PRERELEASE=1' $WAMR_PATH/product-mini/platforms/linux-sgx/enclave-sample/Makefile
